...
class NeuralNetwork():
    ...
    def train(self, train_inputs, train_outputs, num_train_iterations):
        ...
        for iteration in range(num_train_iterations):
            ...
            # multiply the error by input and then by gradient of tanh funtion to calculate
            adjustment = dot(train_inputs.T, error * self.tanh_derivative(output))
            #the adjustment needs to be made in weights
            self.weight_matrix += adjustment # Adjust the weight matrix
# Driver Code
if __name__ == "__main__":
    neural_network = NeuralNetwork()
print ('Random weights at the start of training')
print (neural_network.weight_matrix)
train_inputs = array([[0, 0, 1], [1, 1, 1], [1, 0, 1], [0, 0, 1]])
train_outputs = array([[0, 1, 1, 0]]).T
neural_network.train(train_inputs, train_outputs, 20000)
